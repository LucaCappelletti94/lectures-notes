...
L'algoritmo di Dijkstra viene utilizzato per trovare il cammino più breve tra due nodi di un grafo. L'implementazione dell'algoritmo iniziale ha complessità \(O\rnd{\abs{2}^2}\).
...
\begin{proof}[Correttezza di Dijkstra]
  ...
  \]
  La distanza \(d(u)\) è considerata la distanza minore dalla \textbf{radice} a \(u\) perché se esistesse un cammino più breve e se \(w\) fosse il primo nodo non visitato su quel cammino allora l'ipotesi originale che \(d(w) > d(u)\) non sarebbe più valida.
