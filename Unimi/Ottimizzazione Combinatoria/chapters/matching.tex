...
\begin{proof}[Disuguaglianze duali deboli]
	...
	Siccome \(Y\) copre \(V\), ogni \(x \in X\) incide su almeno un \(y \in Y\).

	Nessun \(y \in Y\) copre contemporaneamente \(x_i, x_j \in X: x_i \neq x_j \) altrimenti i due elementi \(x_i\) ed \(x_j\) risulterebbero adiacenti e quindi \textbf{non potrebbero appartenere} in primo luogo a \(X\), che è un \textbf{isieme stabile}.

	Per ogni \(x \in X\) esiste un distinto \(y \in Y\) che lo copre, e quindi \(\abs{X} \leq \abs{Y}\), che per gli insiemi ottimi \(X^*\) e \(Y^*\) risulta:
	\[
		\a(G) \leq \rho(G)
	\]
	Scambiando il ruolo di \(V\) ed \(E\), si ottiene anche la seconda disequazione \(\mu(G) \leq \tau(G)\).
\end{proof}
\clearpage
...
\begin{theorem}[Estensione di un Matching]
...
\begin{proof}[Estensione di un Matching]
	...
	\begin{description}
		\item \textbf{\(M'\) è un matching:}
		      \begin{enumerate}
			      \item Per i nodi che non sono toccati da \(P\) non è cambiato nulla: su di essi incide un solo arco di \(M\) che ora appartiene anche ad \(M'\).
