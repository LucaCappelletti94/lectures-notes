...
\begin{proof}[Teorema di König (Rizzi, 1999)]
	Dato un grafo \(B\), vale che \(\abs{M_B^*} \leq \abs{C_B^*}\).
	...
	Sia \(v\) uno dei nodi vicini di \(u\):
	\[
		\abs{M_{G\setminus v}^*} < \abs{M_G^*} \Rightarrow \text{per minimalità}\quad \abs{C_{G \setminus v}} < \abs{M_G^*}
	\]

	Ne segue che \(C_{G\setminus v} \cup \crl{v}\) è una cover di \(G\) con cardinalità al più \(\abs{M^*}_G\).

	Possiamo assumere di conseguenza che deve esistere un matching massimo \(M\) di \(G\) privo di lati incidenti a \(v\).

	Sia \(f\) un lato di \(G\setminus M\) incidente a \(u\) ma non a \(v\).

	Sia \(C_{G\setminus f}\) una cover con cardinalità \(\abs{C_{G\setminus f}} = \abs{M_G^*}\).

	Siccome nessun lato di \(M\) è incidente a \(v\), ne segue che \(C_{G\setminus f}\) non contiene \(v\).

	Di conseguenza, \(C_{G\setminus f}\) contiene \(u\) ed è una cover anche di \(G\).
\end{proof}
\clearpage
\subsection{Diagramma della dimostrazione - Teorema di König (Rizzi, 1999)}
\begin{figure}
	\includegraphics[width=0.9\textwidth]{proofs/Teorema_di_König,_Rizzi}
\end{figure}
\clearpage
