...
\begin{figure}
	...
	\begin{subfigure}{0.33\textwidth}
		\begin{tabular}{ |L|LLLL|L| }
			...
			\blue{\bbmc} & t_1      & t_2      & t_3      & t_4      & \blue{\bmu}        \\
			\hline
			s_0          & 11       & 18       & 9        & 0        & \blue{4}           \\
			s_1          & 5        & 14       & 8        & 0        & \blue{7}           \\
			s_2          & 10       & 14       & 16       & 0        & \blue{6}           \\
			s_3          & 1        & 6        & 3        & 0        & \blue{5}           \\
			\hline
			s_4          & \blue{0} & \blue{0} & \blue{0} & \blue{0} & \textbf{\blue{22}} \\
			\hline
		\end{tabular}
		\caption{Duale}
	\end{subfigure}
...
\begin{figure}
	...
	\begin{subfigure}{0.33\textwidth}
		\begin{tabular}{ |L|LLLL|L| }
			...
			\blue{\bbmc} & t_1      & t_2      & t_3      & t_4      & \blue{\bmu}        \\
			\hline
			s_0          & 10       & 12       & 6        & 0        & \blue{4}           \\
			s_1          & 4        & 8        & 5        & 0        & \blue{7}           \\
			s_2          & 9        & 8        & 13       & 0        & \blue{6}           \\
			s_3          & 0        & 0        & 0        & 0        & \blue{5}           \\
			\hline
			s_4          & \blue{1} & \blue{6} & \blue{3} & \blue{0} & \textbf{\blue{32}} \\
			\hline
		\end{tabular}
		\caption{Duale}
	\end{subfigure}
...
\begin{figure}
	\begin{algorithm}[H]
		...
		\While{\(\rnd{\exists i,j: \rnd{c_{ij} - u_i - v_j = 0}\land\rnd{\text{Mate}_i = nil}\land\rnd{\text{Mate}_j = nil}}\)}{
			\(x_{ij} \leftarrow 1\)\;
	...
	\begin{algorithm}[H]
		...
		\For{\(i \in V_1: \text{ Mate}_i = nil\)}{
			...
			\For{\(j \in V_2: \text{ Label}_j = nil\)}{
				\If{\(c_{ij} - u_i - v_i < p_j\)}{
					\(p_j \leftarrow c_{ij} - u_i - v_i\)\;
					\(\pi_j \leftarrow i\)\;
				}
			}
		}
...
\begin{figure}
	\begin{algorithm}[H]
		...
		\uIf{\(v \in V_1\)}{
			Step 3.2.A: Propagazione da \(v \in V_1\) a \(V_2\)\;
		}
		\uElseIf{\(\text{Mate}_v \neq nil\)}{
			Step 3.2.B: Propagazione da \(v \in V_2\) a \(V_1\)\;
		}
		\Else{
			\(\text{Path} \leftarrow v\)
		}
...
\begin{figure}
	\begin{algorithm}[H]
		\SetAlgoLined
		\For{\(j \in V_2: \rnd{Label_j = nil} \land \rnd{c_{vj} - u_v - v_v = 0}\)}{
...
\begin{figure}
	\begin{algorithm}[H]
		\SetAlgoLined
		\If{\(\text{Label}_{\text{Mate}_v} = nil\)}{
		\(\text{Label}_{\text{Mate}_v} \leftarrow v\)\;
		\(L \leftarrow L \cup \crl{Mate_v}\)\;
		\For{\(j \in V_2: \text{Label}_j = nil\)}{
			\If{\(c_{\text{Mate}_v j} - u_{\text{Mate}_v} - v_j < p_j\)}{
				\(p_j \leftarrow c_{\text{Mate}_v j} - u_{\text{Mate}_v} - v_j\)\;
				\(\pi_j \leftarrow \text{Mate}_v\)\;
			}
		}
		}
...
\begin{figure}
	\begin{algorithm}[H]
		...
		\For{\(j \in V_2: \rnd{\text{Label}_j = nil} \land \rnd{p_j = 0}\)}{
			\(\text{Label}_j \leftarrow \rnd{\pi_j, +}\)\;
			\(L \leftarrow L \cup \crl{j}\)\;
		}
...
\begin{figure}
	...
	\begin{subfigure}{0.33\textwidth}
		\begin{tabular}{ |L|LLLL|L| }
			...
			s_3          & 0        & 0        & 0        & 4         & \blue{5}           \\
			\hline
			s_4          & \blue{1} & \blue{6} & \blue{3} & \blue{-4} & \textbf{\blue{40}} \\
			\hline
		\end{tabular}
		\caption{Duale}
	\end{subfigure}
