...
\begin{center}
\begin{tikzpicture}[
        axis/.style={very thick, ->, >=stealth'} ,scale=1.75]
\draw[axis] (-1,0)  -- (6,0) node(xline)[right] {$V_{DS}$};
\draw[axis] (0,-1) -- (0,5) node(yline)[above] {$I_{DS}$};
...
\filldraw [blue] (2, 4) circle (1.5pt) node[above right, blue] {$I_{mos}|_{ow}$};

\filldraw [black] (0.8, 2.51) circle (1.5pt) node[right, black,yshift=0.5] {$I_{work}$};

\filldraw [red] (2, 1.8) circle (1.5pt) node[above right, red] {$I_{L}|_{ow}$};

\filldraw [red] (0,3) circle (1pt) node[left , red] {$I_0$};
\filldraw [red] (5,0) circle (1pt) node[below, red] {$R I_0$};
